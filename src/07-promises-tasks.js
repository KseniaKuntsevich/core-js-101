 ...
 *                                                    //  Ask her again.';
 */
function willYouMarryMe(ans) {
  const promise1 = new Promise(((resolve, reject) => {
    if (ans === false) resolve('Oh no, she said "No".');
    if (ans === true) resolve('Hooray!!! She said "Yes"!');
 ...
 *
 */
function chainPromises(/* promises, act */) {
  ...
  //     const isDone = result.every((response) => (response));

  //     if (isDone) {
  //       clearInterval(timerId);

  //       resolve(result.reduce(act));
  //     }
  //   }, 10);
  // });
}
